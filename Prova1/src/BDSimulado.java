...
public class BDSimulado {
	...
	public BDSimulado() {
		...
		Ligacao l1 = new Ligacao(1, "1245-7896", LocalDateTime.of(2017, 9, 12, 17, 0),
				LocalDateTime.of(2017, 9, 12, 17, 55), "Brasilia", "RJ", c1);
	...
	// ligação por cliente
	ArrayList<Ligacao> ligacoesPorCliente(String nome) {
	...
	public int ligacoesPorUF(String UF) {
		int resultado = 0;
		for (int i = 0; i < this.ligacoes.size(); i++) {
			if (this.ligacoes.get(i).getUFDeDestino().equals(ligacoes)) {
				resultado++;
			}
		}
		return resultado;
	...
	public ArrayList<Integer> ligacoesUFDiferente() {
		...
		for (int i = 0; i < this.ligacoes.size(); i++) {
			if (this.ligacoes.get(i).getUFDeDestino().equals(this.clientes.get(i).getUF())) {
